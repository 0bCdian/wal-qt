...
import { activateImageMode, commitActiveMediaState, state } from "./renderer/state";
import type {
    ImageFitMode,
    ImageRenderingMode,
    LoadRequest,
...
// no-op until wallpaper config push is wired through the load pipeline
function handleConfigPush(payload: unknown): void {
    const typed = payload as { monitor_id?: number; image_fit_mode?: ImageFitMode; image_rendering?: ImageRenderingMode };
